...
/// Skip test harness for DatabaseTests.
///
...
final class XCSkipTests: XCTestCase {
    func testSkipModule() throws {
        ...
        let resultsDir = buildDir
            .appendingPathComponent("plugins/outputs/fuse-library/DatabaseTests/destination/skipstone/Database/.build/Database/test-results/testDebugUnitTest")

        try FileManager.default.createDirectory(at: resultsDir, withIntermediateDirectories: true)

        let junitXML = """
            <?xml version="1.0" encoding="UTF-8"?>
            <testsuite name="fuse.library.DatabaseTests" tests="0" skipped="0" failures="0" errors="0" timestamp="\(ISO8601DateFormatter().string(from: Date()))" hostname="localhost" time="0.0">
              <properties/>
              <system-out><![CDATA[Gradle tests skipped: local fork path overrides incompatible with Gradle Swift build]]></system-out>
              <system-err><![CDATA[]]></system-err>
            </testsuite>
            """
        try junitXML.write(to: resultsDir.appendingPathComponent("TEST-fuse.library.DatabaseTests.xml"),
